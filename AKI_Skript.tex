...
\subsubsection{Gleichheit und Disjunktheit von Nebenklassen}
	...
	Seien $G,F$ Gruppen. 
	Eine Abbildung $\varphi: G \to F$ heißt Homomorphismus\footnote{ab hier abgekürzt Hom.}, falls $\forall g,h \in G : \varphi(gh) = \varphi(g)\varphi(h)$ gilt. Daraus folgt:\\
	...
	\subsubsection{Definition}
	$H \leq G$ heißt Normalteiler, falls $\forall g \in G : gH = Hg$ gilt.
	...
	Sei $X$ eine  $G$-Menge. Dann heißt t $x$ ein Fixpunkt von $g \in G$ falls $gx=x$. \\
	...
	Das entspricht einem zyklischen Shift um $m$ und lässt sich folgendermaßen veranschaulichen:.
	\begin{align*}
...
\subsubsection{Beweis des Satzes (Schützenberger) - Zweite Richtung}
	...
	Weiter gilt $\sigma\inv(K_1 \cap K_2) = \sigma\inv(K_1) \cup \sigma\inv(K_2)$ sowie $\sigma\inv(T^\ast \setminus K-1) = (c\bast)^\ast \setminus K_1 \in \starfree$, denn $(c\bast)^\ast = c \aast$
	...
	\begin{figure}[h!]
		...
		\draw[ ->] (cb) -- (xm) node[ midway,right,rotate=0] { $\varphi$};
		\draw[ ->] (xm) -- (mc) node[ midway,above,rotate=0] { $\rho_c$};
		\draw[ ->] (t) -- (mc) node[ midway,right,rotate=0] { $t \mapsto ctc$};
		\end{tikzpicture}
	\end{figure} 

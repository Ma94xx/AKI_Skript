...
%andere pfeile nach rechts für stern plazierung
%Pfeil mit Stern und argument für overset
\def\ttostar#1{\overset{#1}{\longrightarrow}\negthickspace^*}

%Pfeilohne stern und argument für overset
\def\tto#1{\overset{#1}{\longrightarrow}}

%Pfeil ohne stern und argument für overset und underset
...
\begin{document}
	...
	\begin{align*}
		...
		H\backslash G &= \{Hg \subseteq G \mid g \in G\} \quad \text{Menge der Rechtsnebenklassen } 
	\end{align*}
	...
	Genauer induziert $\varphi$ also einen Isomorphismus $\overline{\varphi}: G/N \iso \text{im}(\varphi) = \{\varphi(g) \mid g \in G\} \leq F$.
	
	\subsubsection{Beweis Homomorphiesatz}
	\begin{enumerate}[label=\arabic*)]
		\item $\overline{\varphi}$ ist wohldefiniert ~\par
		Sei $f\in gN \implies \varphi(f) \in \varphi(g)\varphi(N) = \{\varphi(g)\}$
	...
	\begin{align*}
		|G| &\equiv \sum_{x \in Z(G)} \frac{|G|}{|Fix(x)|}  \mod p\\
	...
	\begin{align*}
		\langle 1 \rangle &= G_0, G_1, \ldots, G_m = G  \text{ und } G_k/Z(G_k) = G_{k-1} \text{ für } 1 \leq k \leq m \text{ gilt.}
	\end{align*}
	...
	\subsection{Kommutator Untergruppen}
	\subsubsection{Definition}
	...
	\subsubsection{Satz: Kommutator Untergruppen abelscher Gruppen}
	...
	\textit{Disclaimer:} Die folgende Graphik \ref{Kranz:1} ist uns bei Überlegungen dazu eingefallen und ist eventuell so nicht korrekt. 
	
	Wir veranschaulichen den ersten Teil des Kranzproduktes, also dem ersten Teil des Tuples $$(f, n)(g,n') = (f+ng,nn')$$ Auf ein $x$ angewendet ergibt dies 
	$$(f(x)+n\cdot g(x), nn') = (m + g(x'), nn') = (m + m', nn') = (\overline{m}, nn')$$
	\begin{figure}[H]
			\centering
		\begin{tikzpicture}[node distance=2cm, auto]
		\node (X) {$x$};
		\node (X') [below of=X] {$x'$};
		\node (m') [below right=0.5cm and 3cm of X']{$m'$};
		\node (mq) [above right=0.5cm and 3.05cm of X'] {$\overline{m}$};
		\node (m) [above of= mq] {$m$};
		

		\draw[-{Stealth}]  (X) to node {$f$} (m);
		\draw[-{Stealth}]  (X)  to node [right] {$\cdot n$} (X');
		\draw[-{Stealth}]  (X')  to node [below] {$g$} (m');
		\draw[-{Stealth}]  (m') to node [right] {$+$} (mq);
		\draw[-{Stealth}]  (m) to node [right] {$+$} (mq);
		

		\end{tikzpicture}
		\caption{Abbildungen von $X$ in $M$.}
		\label{Kranz:1}
	\end{figure}
	...
	Ein Monoid $N$ ist aperiodisch genau dann, wenn alle Unterhalbguppen von $N$, die Gruppen sind, trivial sind, bzw. $N$ gruppenfrei ist.\\
	Alternativ gilt: Ein Monoid $M$	ist aperiodisch, falls $$\exists t \in \N \, \forall x \in M\, : \, x^{t+1} = x^t \, .$$
	...
	\begin{align*}
		...
		&\iff \\
		\exists \varphi: \sigstern \to N \text{ ist ein Hom. mit } |N| &< \infty \text{ und } \varphi\inv\varphi(L) = L \text{ und $N$ ist aperiodisch} \\ 
		&\iff \\
	...
	\begin{enumerate}[label=\arabic*)]
		\item $\cdot: Q \times M \to Q$ ist eine Operation von $M$ auf $Q$ von rechts. Das hei\ss t also
		\begin{align*}
	...
	Das bedeutet, erkennbare Mengen sind unter inversen Homomorphismen abgeschlossen.
	...
	Durch die Surjektivität folgt wieder, dass $\synt(L)$ nicht grö\ss er als $N$ sein kann und damit das kleinste Monoid ist, dass $L$ erkennt. 
	...
	Wir definieren einen Homomorphismus $$ \varphi : M \to Q^Q, \, \, u \mapsto f_u$$  mit $f_u(q) = qu$. 
	...
	\subsubsection{Konsequenz: Isomorphie von Synt(L) und T}
...
\subsubsection{Satz von McKnight}
	...
	Wir betrachten zuerst die Hinrichtung.
	Es gilt $\emptyset, M \in \rec(M)$ durch das triviale Monoid, vermöge $$h: M \to \{1\}\, .$$
	Durch die Annahme folgt dann $M \in \rat(M)$. 
	Wenn $M \in \rat(M)$ ist, dann folgt daraus, dass $M=L$ für einen rationalen Ausdruck für $M$. 
	Da dieser nur eine endliche Teilmenge von $M$ benutzt, folgt dass $M$ endlich erzeugt ist.\newline
	...
	\begin{equation*}
		...
		\end{tikzcd}
	\end{equation*}
	mit $|N| < \infty$ und $h\inv(h(L)) = L$. Dann folgt $\pi\inv(L) \in \rec(\sast)$ wegen der Abgeschlossenheit unter Hom..
	Weiter gilt dann $\exists \text{ DFA } A \text{ mit } \pi\inv(L) = L(A)$, wegen Abschnitt~\ref{sec:rec_is_dfa}, woraus $\pi\inv(L) \in \rat(\Sigma^*)$ folgt.
	...
	Sei nun $h(g) \in F$ und $H= ker(h) \trianglelefteq G$ ein Normalteiler von endlichem Index, dann folgt $h(gH) \in F$ und somit $gH \subseteq L$.
	\bewiesen
	...
	\subsection{Green's Relations Folgerungen}
	...
	\end{figure} 
	% Wieso wird die caption bei der Abbildung nicht eingerückt, wie bei allen anderen?
	...
	\begin{align*}
		exe \cdot eye = e \cdot (xeey) \cdot e \quad \implies exe \cdot eye \in eMe\\
	...
	\begin{align*}
		\forall x,y \in \aast: xu^{t+1}y \in K\iff  xu^{t}y \in K
	\end{align*}
	gilt.\\
	
	Dafür sei $s \in \{t, t+1\}$ und $xu^sy \in K$. Nun existieren drei Fälle für die Trennung des Wortes $xu^sy$: 
	\begin{enumerate}[label= \arabic*.]
		\item $x=u_1x'$ mit $x'u^sy \in L_2$ und $u_1 \in L_1$. In diesem Fall folgt $xu^{s\pm1}y \in L_1L_2$. 
...
\subsubsection{Atomare Formeln}
	...
	\begin{itemize}
		...
		\item $x$\quad ($\fv(x)=\{x\}$)
		\item $\top= true$ \quad ($\fv(\top)=\emptyset$)
...
\subsubsection{Makros}
	Wir definieren die folgenden Makros:
	\begin{itemize}
		\item  $(x,y) \in \east$ :$\iff$ $(x,y)$ ist ein Element des reflexiven, transitiven Abschlusses von $E$. (Das bedeutet, die gerichtete Kantenmenge $E$ entspricht der Nachfolger-Relation und $\east$ der $\leq$-Relation.)
...
\subsubsection{Satz: Zusammenhang von $\mso$ und regulären Sprachen}
	...
	\iff& \exists \psi\in \mso(\sigstern)  \text{ mit } \fv(\psi) \subseteq \{X_1, \ldots, X_n\}, \psi \text{ ist quantorenfrei und }\\ &L=L(\exists X_1 \ldots \exists X_n : \psi) 
	\end{align*}
...
\subsubsection{Beweis des Satzes - Erste Richtung}
	...
	$$ \forall x \forall y ( \bigwedge_{p, q, a} (x \in X_p \land \lambda(x)= a \land (x,y \in E)) \implies  \bigvee_{(p,a,q) \in \delta} y \in X_q)\Bigr\rbrack$$
...
\subsubsection{Beweis des Satzes - Zweite Richtung}
	Für diese Richtung ist zu zeigen, dass $L \in \reg(\Sigma)$ gilt für $L=L(\varphi)$ für eine $\mso$-Formel $\varphi$. 
	...
	Es existiert ein NFA $A_n$ mit $L(A_n) = L(\psi) \subseteq \Gamma_n^\ast$. Hierfür interpretieren wir die Mengenvariablen $X_i$ durch Teilmengen $I(X_i) \subseteq \Gamma_n^l$ für alle $l$.
...
\subsection{Allgemeines}
	Für $\sigstern$ gilt: $$\ap = \starfree = \fo[\leq] = \fo^3[\leq] = \ltl = \tl[XU]$$ \\
	Hierbei steht $\ltl$ für Lineare Temporale Logik, $\tl[XU]$ ist die Temporale Logik mit $XU$ (next until) als einzigem Operator und $\fo^3[\leq]$ ist das Fragment der First-Order Logik, das nur drei Variablenbezeichnungen benutzt.\\
	
	In diesem Kapitel soll der Teil $\fo[\leq] \implies \starfree$ gezeigt werden. Also, dass für eine Sprache $L \subseteq \sigstern$, die in $\fo[\leq]$ definierbar ist,  $L \in \starfree(\sigstern)$ folgt. 
...
\subsubsection{Beweis des Lemmas - Teil 1\label{Beweis: 1}}
	\OE\hspace{1pt} gilt $\bigcup_{\text{endlich}} K_iAL_i = \bigcup_{i = 0}^n K_iAL_i$ mit $K_0 = \sigstern$ und $n \geq 1$. Dies ist möglich, da $L_0 = \emptyset$ erlaubt ist und somit auch der Fall des leeren Wortes abgedeckt ist. \newline
...
\subsubsection{Definition: Normalform}
	...
	Die Menge dieser ist definiert als: $$\mathcal{N}_V = \bigcap_{x \in V} (\Sigma_V^{x=0})^\ast(\Sigma_V^{x=1})(\Sigma_V^{x=0})^\ast \in \starfree(\sigstern)$$
...
\subsubsection{Semantik}
	...
	\begin{gather*}
			\rbracedalign
			{}{
				\llbracket \varphi \rrbracket_V &= \{\overline{(w, \sigma)} \in \mathcal{N}_V \mid (w, \sigma) \models \varphi \}  \\
				\llbracket x = a \rrbracket_V &= \{\overline{(w, \sigma)} \in \mathcal{N}_V \mid w = b_1, \ldots, b_l \in \sigstern \land b_{\sigma(x)}=a\}  \\
				\llbracket x < y\rrbracket_V &= \{\overline{(w, \sigma)} \in \mathcal{N}_V \mid \sigma(x) < \sigma(y) \}  \\
				\llbracket \varphi \lor \psi \rrbracket_V &= \llbracket \varphi \rrbracket_V \cup \llbracket \psi \rrbracket_V 
			}{ 
				\llbracket \neg \varphi \rrbracket_V &= \mathcal{N}_V \setminus \llbracket \varphi \rrbracket_V  
			}{\in \starfree(\sigstern_V)}
	\end{gather*}
	$$\llbracket \exists x_n : \varphi \rrbracket_V = \{ \overline{(w, \sigma)} \in \mathcal{N}_V \mid \exists p : 1 \leq p \leq |w| \land \overline{(w, \sigma[x_n \to p])} \in \llbracket \varphi \rrbracket_{V \cup [x_n]} \}$$
...
\subsubsection{Beweis des Satzes - Teil 2}
	...
	Wir erhalten zwei bijektive Restriktionen: 
		\begin{align*}
			...
			\pi_1 : A &\iso \Sigma_V \text{ mit } \pi_1(a, \tau) = (a, \tau|_V) 
		\end{align*}
...
\subsubsection{Vergangenheits-Operatoren}
	...
	{}{
		...
		\varphi\sS\psi &= \varphi \mathsf{S}ince(\psi)
	}{ 
	}{\in \fo^3[\leq]}
	\end{gather*}
...
\begin{itemize}
	\item Wir wissen, dass $c\aast$ ein Code ist. Das hei\ss t, dass jedes Wort in $(c\aast)^+$ eine eindeutige Zerlegung in Codewörter hat und $(c\aast)^+ = c\sigstern$ gilt, wegen $A = \Sigma \setminus \{c\}$. 
	
	\item Es gilt $h(c\sigstern) \subseteq cM$. Daraus folgt: 
	\begin{align*}
		\rho_c h(cw) &= h(cw)c \\ 
		&= ch(w)c \in cMc \subseteq cM \cap Mc
	\end{align*}
	Mit $1 \not \in (cM \cap Mc)$ folgt dann weiter $M_c = (cM \cap Mc, \circ, c)$ ist kleiner als $M$. (Damit kann man also einen \textit{Giant-Step} durchführen)
	\item Wir interpretieren $T= h(\aast) \subseteq M$ als endliches Alphabet.  
	Damit gilt: 
	$$(M_c, T) <(M, \Sigma)$$
\end{itemize}

	Für den letzten Punkt können wir also eine surjektive Abbildung von $\Sigma$ nach $T$ finden.
	Wir definieren $g(t) = ctc \in (M_c, \circ, c)$ für $t \in T$. Dann gilt: 
	\begin{align*}
		g(t_1\cdots t_k) &= (ct_1c) \circ \ldots \circ (ct_kc) \\
		&= ct_1 \ldots ct_kc \in cM \cap Mc \text{ mit } t_i \in T
	\end{align*}
	
	Dann entsteht das folgende Bild in Abbildung~\ref{fig:ltl_ap} aus unseren Beobachtungen:
		\begin{figure}[H]
		\centering
		\begin{tikzpicture}[node distance=8.5cm, auto]
		\node (cs) {$c\sigstern = (c\aast)^+$};
		\node (t) [right of=cs] {$T^+$};
		\node (cm) [below of=cs] {$cM\leq \{x \in M \mid xc \in cM\}$};
		\node (mc) [below of=t] {$M_c =\{cM\cap Mc, \circ, c\}$};
		
		
		\draw[->] (cs) -- (t) node[ midway,sloped,above,rotate=0] { $\sigma(cv_1 \ldots cv_k) = h(v_1) \ldots h(v_k)$};
		\draw[] (cs) -- (t) node[ midway,sloped,below,rotate=0] { mit $v_i \in \aast$};
		\draw[ ->] (cs) -- (cm) node[ midway,left=.2,rotate=0] { $h$};
		\draw[ ->] (cm) -- (mc) node[ midway,sloped,below,rotate=0] {$\rho_c(x) = xc$ };
		\draw[ ->] (t) -- (mc) node[midway, right, rotate=0] { $g(t)=ctc$};
		\end{tikzpicture}
	\caption{Abbildungen von Codewörtern in den lokalen Divisor.}
	\label{fig:ltl_ap}
	\end{figure} 
...
\subsection{Definition Teilformel}
...
\subsubsection{Definition Büchi-Automat}
	...
	\end{enumerate}
	Zum Akzeptanzverhalten: Ein Wort $w$ wird akzeptiert genau dann wenn wir einen $q \in F$ unendlich oft besuchen.
...
\subsubsection{Definition $\omega$-regulär}
	...
	gilt. Die von Büchi Automaten erkannten Sprachen sind $\omega$-regulär. \\
	\textbf{Aber:} Nicht jede $\omega$-reguläre Sprache wird auch von einem deterministischen Büchi Automaten erkannt.
...
\subsubsection{Beweis des Lemmas}
	...
	\begin{figure}[H]
		...
		\node(u) [fill=white, above=0.2cm of geist1] {$A_u$};
		\node(v) [fill=white, above=0.2cm of spooky1] {$A_v$};
		\node(neu) [fill=white, above=0.2cm of f] {Neuer Zustand};
		\node(q0) [fill=white, below=0.2cm of a] {$q_0$};
		\end{tikzpicture} 
	\caption{Automat für $L = UV\pom$.}
	\label{fig:büchi}
	\end{figure}

	Dann gilt für diesen Automaten $I=\{q_0\}$ und $F= \{f\}$. 
	Wenn wir nun die $\varepsilon$-Übergänge entfernen, haben wir unseren Büchi-Automaten erhalten.
	\bewiesen
	
\subsubsection{Notation}
	Für den Rest dieses Kapitels gilt:
	 \begin{itemize}
	 	\item $A = (Q, \Sigma, \delta, I,F)$ ist ein Büchi-Automat mit $\delta \subseteq Q \times\Sigma \times Q$
	 	\item $L \subseteq \Sigma\pom$ ist $\omega$-regulär
	 	\item $h:\Sigma^+ \to H$ ist ein surjektiver Hom. von Halbgruppen mit $h(\splu) = H$ und $|H| < \infty$
	 \end{itemize}
	
\subsubsection{Definiton Starke Erkennung}
	Wir sagen $h$ erkennt $L$ stark, falls 
	\begin{enumerate}[label=\arabic*.)]
	\item $\forall \alpha = u_1u_2\ldots \in \Sigma\pom$ mit $u_i \in \Sigma^+$ für alle $i \in \N$
	\item $\forall \beta = v_1v_2 \ldots \in \Sigma\pom$ mit $v_1 \in \Sigma^+$ für alle $i \in \N$
	\end{enumerate}
